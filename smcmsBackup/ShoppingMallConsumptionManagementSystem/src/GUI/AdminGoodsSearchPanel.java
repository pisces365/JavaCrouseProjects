...
class AddAndEditPanel extends JFrame
{
	...
	{	
		...
		if(items != null)//更新数据
		{
			...
			goodsNumber = new InformationItemAdmin("商品编号：",items.get(0),false);
			goodsName = new InformationItemAdmin("名称：",items.get(1),false);
			manufacturer = new InformationItemAdmin("制造商：",items.get(2));
			dateOfProduction = new InformationItemAdmin("生产日期：",items.get(3),0.1);
			price = new InformationItemAdmin("价格(元)：",items.get(4));
			discount = new InformationItemAdmin("折扣率(%)：",0.1,Integer.parseInt(items.get(5)));
			inventory = new InformationItemAdmin("库存：",items.get(6));
			productIntroduction = new InformationItemAdmin("商品简介：",items.get(7));
			remarks = new InformationItemAdmin("备注：",items.get(8));
		}
		else//添加数据
		{
			this.setTitle("商品信息添加");
			goodsNumber = new InformationItemAdmin("商品编号：",0);
			goodsName = new InformationItemAdmin("名称：",0);
			manufacturer = new InformationItemAdmin("制造商：",0);
			dateOfProduction = new InformationItemAdmin("生产日期：","2010-1-1",0.1);
			price = new InformationItemAdmin("价格(元)：",0);
			discount = new InformationItemAdmin("折扣率(%)：",0.1,100);
			inventory = new InformationItemAdmin("库存：",0);
			productIntroduction = new InformationItemAdmin("商品简介：",0);
			remarks = new InformationItemAdmin("备注：",0);
		}
...
public class AdminGoodsSearchPanel extends JPanel{
	...
	private JButton searchItemButton,DeleteButton,AddButton,EditButton;
